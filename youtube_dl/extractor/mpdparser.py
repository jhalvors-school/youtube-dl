...
class MPDParser:
    ...
    def parse(self):
        mpd_duration = parse_duration(self.mpd_doc.get('mediaPresentationDuration'))
        formats = []
        for period in self.mpd_doc.findall(self._add_ns('Period')):
            period_duration = parse_duration(period.get('duration')) or mpd_duration
            period_ms_info = self.extract_multisegment_info(period, {
                'start_number': 1,
                'timescale': 1,
            })
            for adaptation_set in period.findall(self._add_ns('AdaptationSet')):
                if self.is_drm_protected(adaptation_set):
                    continue
                adaption_set_ms_info = self.extract_multisegment_info(adaptation_set, period_ms_info)
                for representation in adaptation_set.findall(self._add_ns('Representation')):
                    ...
                    if content_type == 'text':
                        ...
                        base_url = ''
                        for element in (representation, adaptation_set, period, self.mpd_doc):
                        ...
                        representation_ms_info = self.extract_multisegment_info(representation, adaption_set_ms_info)
                        ...
                        if 'segment_urls' not in representation_ms_info and 'media' in representation_ms_info:
                            ...
                            media_template = prepare_template('media', ('Number', 'Bandwidth', 'Time'), representation_ms_info, representation_id)
